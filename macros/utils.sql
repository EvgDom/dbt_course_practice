...
{# Скопировал макрос drop_old_relations с урока 10 #}
{% macro drop_old_relations(dryrun=False) %}
    ...
    {% if execute %}
        ...
        SELECT
            'DROP IF EXISTS ' || RELATION_TYPE || ' ' || RELATION_NAME || ' CASCADE;' as DROP_COMMANDS
            {# Доманов Е - добавил IF EXISTS и CASCADE для удаления зависимых обектов #}
        FROM
            MODELS_TO_DROP;
        {% endset %}
